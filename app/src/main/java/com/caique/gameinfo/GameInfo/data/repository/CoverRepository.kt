...
class CoverRepository {
    ...
    fun getCover(gameId: Int): MutableLiveData<List<CoverResponse>> {
        val coverResponseMutableLiveData: MutableLiveData<List<CoverResponse>> = MutableLiveData()
        val requestBody: RequestBody = RequestBody.create(MediaType.parse(REQUESTBODY_TAG),
            WHERE_GAME + gameId + SEMICOLON + ALL_FIELDS)
        service.getCovers(USER_KEY, requestBody).enqueue(object : retrofit2.Callback<List<CoverResponse>> {
            override fun onResponse(call: Call<List<CoverResponse>>, response: Response<List<CoverResponse>>) {
                coverResponseMutableLiveData.postValue(response.body())
            }
            override fun onFailure(call: Call<List<CoverResponse>>, t: Throwable) {
